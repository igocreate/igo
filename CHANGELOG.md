...
- **Added**: `config.onCrash(err)`, an async callback invoked after an uncaught exception, within the one second the process already waits before exiting — where a project flushes its telemetry exporters. The spans of the request or job that crashed were lost with the process, and they are the error traces a sampler keeps. It runs once the failed response is flushed, since the request's span only ends then; the second stays a ceiling, and a rejection is logged. Distinct from `onShutdown`, which may drain pools that have no business in a crash.
- **Fixed**: the `fullstack` skeleton no longer stops OpenTelemetry on its own `SIGTERM` handler, which raced igo's shutdown and closed the exporters before the requests in flight had finished. `instrumentation.ts` exports `stopTelemetry`, which `app.ts` hands to `config.onShutdown` and `config.onCrash`.
- **Removed**: `config.exitOnUncaughtException`, added in 6.3.1. Set to `false`, it kept the process serving after an uncaught exception raised during an answered request — precisely a throw from a timer, a callback or an unlistened `error` event, which can leave a stream or a pool half-broken, while errors in async routes never reach that handler. An uncaught exception now always exits, as it did before 6.3.1; a project still setting the key is restarted by its process manager instead.
- **Fixed**: the cache connection is pinged every minute (`config.redis.pingInterval`). A managed Valkey closes a connection idle for 10 minutes; igo then logged `Socket closed unexpectedly` and, on reconnecting, flushed the whole cache database — so an instance without traffic wiped the cache of every instance sharing it, every 10 minutes.
...
- **Changed**: the `fullstack` skeleton names every signal by project, for a Grafana stack shared between projects: `service_namespace` on the API's metrics and traces (`{project.name}` by default, `OTEL_RESOURCE_ATTRIBUTES` overrides it), on the browser's through Faro's `app.namespace`, and on what Alloy collects itself. The dashboard and the alerts filter on it rather than on `job`, and alerts group by `service_name`. The front is named `<project>-front`, from `VITE_APP_NAME` — formerly `VITE_FARO_APP_NAME`, whose default was a leftover `'audit'`.
- **Changed**: the `fullstack` skeleton's Alloy agent sets the `environment` label of the logs from the machine's `ENVIRONMENT` rather than reading it in the line, and starts following a log file from its end.
- **Changed**: the `fullstack` skeleton exports and scrapes metrics every 60 s instead of 15 s and 30 s — Grafana Cloud bills per data point per minute and series, one included.
- **Fixed**: a blank or invalid `VITE_FARO_SAMPLE` falls back to the default share instead of silently disabling sampling (`Number('')` is `0`).
- **Fixed**: the `fullstack` skeleton no longer claims Faro's session id is kept in memory: `persistent: false` stores it in `sessionStorage`, which the project has to weigh under the ePrivacy consent rules.
- **Added**: the `fullstack` dashboard gets an « external dependencies » section — codes and p95 latency of outgoing HTTP calls per `server_address`, a failed connection shown by its `error_type` — and its `$env` variable now filters every application query and the logs; it filtered nothing.
- **Changed**: the `fullstack` dashboard shows p95 latency per route full width with a table legend, disk throughput and queue per machine on physical devices only, every section expanded, and 0 % rather than « No data » when there is no 5xx.
- **Changed**: the `fullstack` skeleton's Alloy identifiers are in English. The variables become `INSTANCE_NAME` (was `METRIQUES_INSTANCE`), `HOST_ROLE` (was `METRIQUES_ROLE`) and `PROBE_URL` (was `SONDE_URL`): a project generated earlier renames them where Alloy runs. The examples target Alloy 1.19 (`trace_conditions`, `file_match` in `loki.source.file`), and log lines carry the machine's `instance`.
- **Fixed**: the `fullstack` skeleton reports the same `service.version` in traces and metrics as in logs: `APP_VERSION`, else the package version, instead of a hard-coded `0.0.1`.
- **Changed**: the `fullstack` README keeps the principles and the naming convention; setting up Grafana Cloud and the alerts moved to `deploy/README.md`, and the Alloy examples lost their duplicated sections and stale comments.
