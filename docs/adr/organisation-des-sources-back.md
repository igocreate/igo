...
`@igojs/server` impose deux fichiers et une convention d'aliases :
...
- **Aliases de modules** — définis dans `package.json`, câblent les imports :
...
Les aliases imposent les dossiers de premier niveau mais pas l'organisation interne. On peut en ajouter de nouveaux sans toucher aux existants.
...
**Règle d'import entre features** : une feature peut importer un modèle ou un service d'une autre feature (`require('@features/dossiers/Dossier')`). Les features ne sont pas des silos étanches — l'organisation porte la propriété, pas l'isolation. Si un modèle est importé par la majorité des features, il migre dans `shared/models/`.
...
exports.create = async (req, res) => {
  const payload = dto.validate(req.body);
  const dossier = await FolderService.create(payload);
  res.status(201).json(dto.serialize(dossier));
};
```

**DTO** — sérialisation sortante + validation entrante (Zod) :
...
exports.validate = (body) => CreerDossierSchema.parse(body);
...
Le DTO est la barrière entre le modèle ORM et l'API. **Le front ne voit jamais un modèle ORM brut.** Si un domaine grossit, un sous-dossier `dto/` peut accueillir plusieurs sérialiseurs.
...
Le error handler actuel fait `res.status(500).render('errors/500')` — du HTML. Sur les routes API, il doit répondre en JSON.
...
### Validation avec Zod — middleware optionnel

Un middleware de validation générique dans `@igojs/server` :

```js
const { validate } = require('@igojs/server');
const { CreerDossierSchema } = require('./dossiers.dto');

router.post('/', validate(CreerDossierSchema), controller.create);
```

Le middleware parse le body avec Zod, renvoie une 400 structurée si invalide, ou appelle `next()` avec le body validé.
