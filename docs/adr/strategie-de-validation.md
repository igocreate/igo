...
- **Le middleware vit dans `@igojs/server`**, valide le corps de la requête contre un **schéma DTO déclaré indépendant du modèle de persistance**, et renvoie des **erreurs structurées** dans un format stable que le front affiche sans transformation. Rien n'est ajouté à `@igojs/db`.
