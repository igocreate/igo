...
- **Authentification fournie par igo** — candidat v7 : garde, chargement de l'utilisateur, parcours de connexion (mot de passe avec hachage et limitation, réinitialisation, OIDC pour FranceConnect et ProConnect, second facteur). À extraire du code de ladom ; outil à évaluer entre `openid-client`, `better-auth` et Passport. En v6 le squelette ne fournit que la garde sur la session.
